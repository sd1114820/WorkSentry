...
func (h *Handler) handleWorkEndAfterReport(ctx context.Context, employeeID int64, departmentID sql.NullInt64, payload ClientReportRequest, now time.Time) (bool, error) {
	if h.DB == nil {
		return false, fmt.Errorf("数据库未初始化")
	}

	session, err := h.Queries.GetOpenWorkSessionByEmployee(ctx, employeeID)
	...
	if departmentID.Valid && departmentID.Int64 > 0 {
		tpl, err := h.Queries.GetEnabledCheckoutTemplateByDepartment(ctx, departmentID.Int64)
	...
	if departmentID.Valid && departmentID.Int64 > 0 {
		rule, thresholds, configured, err = h.loadDepartmentRules(ctx, departmentID.Int64)
		if err != nil {
	...
	if configured {
		breakSummary, err = h.calcBreakSummary(ctx, employeeID, session.StartAt, now)
		if err != nil {
			return false, fmt.Errorf("计算休息时长失败")
		}
		statusTotals, err = h.calcStatusTotals(ctx, employeeID, session.StartAt, now)
		if err != nil {
